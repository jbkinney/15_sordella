...
# Make sure that extensions on files are the same, i.e. that they come from the
# same traunch. 
...
def gapless_alignment(seq1, seq2):
    ...
    k = -1
    i = 0
    while k < 0 and i < L1-overlap_length:
        k = a2_seq.find(seq1[i:overlap_length+i])-i
        i += 1
...
#     num = int(atoms[0])
#     sample = atoms[1]
#     num_to_sample_dict[num] = sample
#     output_file = '%s/%s/observed_seqs.%s'(output_dir,sample,extension)
...
while not stop:
    ...
    if num_reads%5000 == 0 and num_reads > 0:
        ...
        print 'Total time: %d sec, Total reads: %d, Successes: %d%%'%(total_time, num_reads, successes_pct)
        #print time_dict
    ...
    fwd_seq = region_seq[:primer_length]
    rev_seq = rc(region_seq[-primer_length:])

    if not ((read1[:primer_length]==fwd_seq or read1[:primer_length]==rev_seq) and (read2[:primer_length]==fwd_seq or read2[:primer_length]==rev_seq)):
        continue
    ...
    rc_consensus = rc(consensus)
    # Test forward sequence
    if (region_seq[:primer_length]==consensus[:primer_length]) and \
        (region_seq[-primer_length:]==consensus[-primer_length:]):
            o_f.write(consensus+'\n')
            num_successes += 1
    # Test reverse sequence
    elif (region_seq[:primer_length]==rc_consensus[:primer_length]) and  \
        (region_seq[-primer_length:]==rc_consensus[-primer_length:]):
            o_f.write(rc_consensus+'\n')
            num_successes += 1
    # If no success, then fail
    else:
        num_fails_alignment += 1
        #print '---'
        #print 'c: ' + consensus
...
#fails_region_pct = 100*num_fails_region/num_reads
#fails_alignment_pct =  100*num_fails_alignment/num_reads
r1_base = r1_file.split('/')[-1]
r2_base = r2_file.split('/')[-1]
s = 'r1_file\tr2_file\ttot_reads\tsuccess_pct\n'
s += '%s\t%s\t%d\t%0.2f\n'%(r1_base, r2_base, num_reads, successes_pct)
#s = 'r1: %s\nr2: %s\n'%(r1_file, r2_file)
#s += 'Total time: %d sec, Total reads: %d, Successes: %d%%, Sample fails: %d%%, Region fails: %d%%, Alignment fails: %d%%\n'%(total_time, num_reads, successes_pct, fails_sample_pct, fails_region_pct, fails_alignment_pct)
g.write(s)
g.close()
